...
const EditTask: React.FC<EditTaskProps> = ({ row, setRows }) => {
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const data = await (await fetch(`http://localhost:8000/api/todos/${row.id}/`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <>
      ...
      <AddEditModal show={modal} row={row} toggle={toggle} handleSubmit={handleUpdate} />
    </>
  );
}
